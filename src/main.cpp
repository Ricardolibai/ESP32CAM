...
const char *ssid = "CU_Ricardo";
const char *password = "1213lizhisai";
...
//   else
//   {
//     digitalWrite(relay_pin, LOW); //关闭（通电）继电器，使门解锁
//     ledflag = 0;
//     delay(1000);
//     Serial.println(444);
//     client.send("door_open");
          ...
          if (g_state == START_RECOGNITION && (st_face_list.count > 0))
          {
            ...
            if (f)
            {
              ...
              if (millis() - interval > door_opened_millis)
              {
                if (ledflag == 0)
                {
                  sprintf(recognised_message, "开启 - %s", f->id_name);
                ...
                {
                  sprintf(recognised_message, "关闭 - %s", f->id_name);
                  digitalWrite(relay_pin, LOW);
                  ledflag = 0;
                  Serial.print(f->id_name);
